...
const {
    ...
    sortObjectsByNonNumericFieldValues,
    CODING_CHALLENGE_DIFFICULTY_ORDER,
                ...
                documentObjectArrayReduce(
                    sortObjectsByNonNumericFieldValues(
                        codingChallengesData.map((c) => {
                            c.lastAnsweredQuestion =
                                c.lastAnsweredQuestion[currentUserID] || -1;
                            c.isQuizCompleted =
                                c.lastAnsweredQuestion >=
                                c.quizQuestions.length;

                            const { quizQuestions, ...dataWithoutQuestions } =
                                c;
...
exports.updateUserChallengeProgress = (req, res, next) => {
    ...
    if (req.body?.newProgressValue != null) {
            ...
            .firebaseWrite(
                `codingChallenges/${req.codingChallengeID}`,
                {
                    lastAnsweredQuestion: {
                        [`${currentUserID}`]: req.body.newProgressValue,
                    },
                },
                next
            )
        ...
        return throwError(
            400,
            `Missing expected value in request body: newProgressValue`,
            next
        );
    }
};
        ...
        firestoreService
            .firebaseCreate(
                `codingChallenges`,
                {
                    ...
                    quizDescription:
                        req.body.quizDescription || "Try the quiz now!",
                    lastAnsweredQuestion: {},
                    quizQuestions:
                        req.body.quizQuestions?.map((question) => ({
                            questionBody:
...
exports.deleteCodingChallenge = (req, res, next) => {
    ...
    firestoreService
        .firebaseDelete(`codingChallenges/${req.codingChallengeID}`, next)
        .then(() => {

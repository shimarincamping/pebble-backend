...
exports.firebaseCreateUser = async (data, next, uid) => {
    try {
...
exports.firebaseWrite = async (path, data, next, overwriteData = false) => {
    try {
        return await setDoc(doc(db, ...path.split("/")), data, {
            merge: !overwriteData,
...
exports.firebaseRead = async (path, next) => {
    try {
        const resp = await getDoc(doc(db, ...path.split("/")));
        return resp.data();
    } catch (err) {
        return throwError(500, err, next);
    }
...
exports.firebaseReadQuery = async (path, queryParams, next) => {
    try {
        const resp = await getDocs(
            query(collection(db, ...path.split("/")), ...queryParams)
        );
        return resp.docs.map((doc) => ({ docId: doc.id, ...doc.data() }));
    } catch (err) {
        return throwError(500, err, next);
...
exports.firebaseDelete = async (path, next) => {
    try {
        return await deleteDoc(doc(db, ...path.split("/")));
    } catch (err) {
        return throwError(500, err, next);
    }
};
